...
| --import.remote-state.compact                         | `IMPORT_REMOTESTATE_COMPACT`                       | Enables remote state in a compacted form (zipped and Base64 encoded) to reduce the RealmRepresentation object size, which maybe useful in environments with limited request body size gateway policies. The compact state is revertible and will be stored in a plain text format, as soon as the flag is set in `false` again.                                                                                                    | `false`  |                               |
